...
var CosmicPlanetarium = (function() {
  ...
  // the music box. Reads the player's actual rendered height so it
  // tracks expanded ↔ minimal transitions and any future size
  // changes (e.g. wrapped labels on narrow viewports).
  ...
  function injectPlayer() {
    ...
    if (p) {
      ...
      // Keep hint pinned to the music box's top edge as the player
      // grows/shrinks (slide-in, minimal toggle, etc).
      if (typeof ResizeObserver !== 'undefined') {
        playerResizeObserver = new ResizeObserver(updateHintPosition);
        playerResizeObserver.observe(p);
      } else {
        // Fallback for older browsers: snapshot a few times across the
        // slide-up + toggle animations.
        setTimeout(updateHintPosition, 50);
        setTimeout(updateHintPosition, 900);
        setTimeout(updateHintPosition, 1700);
      }
    }
  }
