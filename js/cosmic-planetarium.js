...
var CosmicPlanetarium = (function() {
  ...
  function injectPlayer() {
    if (!modal || typeof CosmicPlayer === 'undefined' || !openOpts || !openOpts.meta) return;
    ...
    if (typeof getRarityTier === 'function') {
      ...
      modal.style.setProperty('--rarity-color', tier.color);
    }
    CosmicPlayer.inject(modal, openOpts.meta);
    var p = modal.querySelector('.cosmic-player');
    if (p) {
      p.style.position = 'fixed';
      p.style.left = '0'; p.style.right = '0'; p.style.bottom = '0';
      p.style.margin = '0';
      p.style.zIndex = '20';
    }
  }
