...
function renderCert(meta, options) {
  ...
  // mode was dropped in V1 — kept only for legacy V4 dev-era records
  ...
  // cfg_scale (V1) — fall back to legacy `cfg` for V4 dev-era records
  var _cfgVal = (meta.cfg_scale !== undefined) ? meta.cfg_scale : meta.cfg;
  if (_cfgVal !== undefined) { _push('cfg_scale', 'CFG', _cfgVal); _claimedKeys['cfg'] = 1; }
  ...
  // model (V1) — fall back to legacy `unet` for V4 dev-era records
  var _modelVal = meta.model || meta.unet;
  if (_modelVal) { _push('model', 'Model', _modelVal, 3); _claimedKeys['unet'] = 1; }
  ...
  var _systemKeys = {
    ...
    // Rosetta Stone — system-owned doc string, never a generation param
    about: 1,
  };
